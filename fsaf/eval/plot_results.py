...
def plot_results(path, logplot=False,name="",limit_y = None):
    ...
    for result in results:
        ...
        df.to_csv(os.path.join(path,result.policy+".csv"), index=False)
    ...
    fig.savefig(fname=os.path.join(path, "plot{}.png".format(name)))
    plt.close(fig)
